		      ...
		      struct net_buf_simple *buf)
{
	...
	printk("[DEVICE]: %s, AD evt type %u, Tx Pwr: %i, RSSI %i "
	       ...
	       bt_addr_le_str(info->addr), info->adv_type, info->tx_power, info->rssi,
...
{
	/* Passive scan, no duplicate filtering, 30 ms window / 60 ms interval -
	 * matches what BlueZ configures on the HCI firmware under test, so the
	 * RSSI figures are directly comparable.
	 */
